...
Status CompactionJob::Run() {
  ...
  for (auto& state : compact_->sub_compact_states) {
    ...
    if (state.compaction_job_stats.file_read_nanos < (1ll << 40))
...
Status CompactionJob::AddKeyValueFromKVBuffer() {
  SubcompactionState* sub_compact = &compact_->sub_compact_states[0];
  ColumnFamilyData* cfd = sub_compact->compaction->column_family_data();
  ROCKS_LOG_INFO(db_options_.info_log,
                   "[%s] [JOB %d] SBC worker start",
                   cfd->GetName().c_str(), job_id_);
  ...
  while (sbc_running_ || sbc_key_value_buffer_->size()) {
    ...
    add_from_buffer_cnt++;
    s = sub_compact->AddFromBuffer(*kv_node, open_file_func, close_file_func, SBC_iter_->output_to_penultimate_level());
    if(!s.ok()) {
      break;
    }
  }
...
Status CompactionJob::FlushSSTable(SubcompactionState* sub_compact) {
  ...
  while (sbc_running_ || files_need_flush_.get_length()) {
    ...
    // 还没开始刷
    auto unflushed_size = sst_file->outfile_->GetUnflushedSize();
    if (sst_file->io_ctx_.io_state_ == kInit) {
      // 还没开始刷磁盘
      // 发起刷盘请求
      sst_file->io_ctx_.cond_var_ = &cv_kv_buf_;
      sst_file->io_ctx_.weight_ = (is_sbc_job_) ? 3 : 4;
      if(compact_->compaction->output_level() == 1) {
        sst_file->io_ctx_.weight_ = 2;
      }
      ...
      continue;
    } else if (sst_file->io_ctx_.io_state_ == kSubmitted) {
      // 当前文件的IO还没执行完
      wait_io_cnt++;
      files_need_flush_.push(sst_file);
      if(wait_io_cnt == files_need_flush_.get_length()) {
        cv_kv_buf_.Wait();
        wake_up_cnt++;
        wait_io_cnt = 0;
      }
      continue;
    } else if (sst_file->io_ctx_.io_state_ == kDone) {
      // 文件全刷到磁盘上了
      sst_file->outfile_->FinishIOCtx(&sst_file->io_ctx_);

      unflushed_size = sst_file->outfile_->GetUnflushedSize();
      if (unflushed_size) {
        sst_file->io_ctx_.cond_var_ = &cv_kv_buf_;
        sst_file->io_ctx_.weight_ = (is_sbc_job_) ? 3 : 4;
        if(compact_->compaction->output_level() == 1) {
      ...
      uint64_t output_number = meta->fd.GetNumber();
      // std::cout << "Flush file success number: " << output_number << ", apply and close file\n";
      ...
      ROCKS_LOG_ERROR(db_options_.info_log,
                "[%s] [JOB %d] Flush SSTable %" PRIu64 ", Flushed : %" PRIu64 ", need flush: %" PRIu64,
                cfd->GetName().c_str(), job_id_, sst_file->output_->meta.fd.GetNumber(), 
                sst_file->outfile_->GetFlushedSize(), 
                sst_file->outfile_->GetBufferLeftSize());
      abort();
    }
  }
