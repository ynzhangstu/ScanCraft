...
#include "writable_file_writer.h"
...
IOStatus WritableFileWriter::GetIOCtx(SBCIOContex *io_ctx) {
  if (seen_error()) {
    assert(false);
    return IOStatus::IOError("Writer has previous error.");
  }
...
IOStatus WritableFileWriter::FinishIOCtx(SBCIOContex *io_ctx) {
  assert(io_ctx);
  const size_t alignment = buf_.Alignment();
  const size_t file_advance = io_ctx->file_advance_;
  const size_t leftover_tail = io_ctx->leftover_tail_;

  if (!io_ctx->s_.ok()) {
    buf_.Size(file_advance + leftover_tail); // NOTE: 去掉pad的部分
    set_seen_error();
    return io_ctx->s_;
  }

  uint64_t cur_size = flushed_size_.load(std::memory_order_acquire);
  flushed_size_.store(cur_size + io_ctx->data_.size(), std::memory_order_release);
  if (io_ctx->s_.ok()) {
    // Move the tail to the beginning of the buffer
    // This never happens during normal Append but rather during
    // explicit call to Flush()/Sync() or Close()
