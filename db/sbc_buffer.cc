...
#include "sbc_buffer.h"


namespace ROCKSDB_NAMESPACE {

void ROCKSDB_NAMESPACE::SBCBuffer::SubmitIOCtx(SBCIOContex *io_ctx) {
...
void SBCBuffer::WorkerFunction() {
  ROCKS_LOG_INFO(db_options_.info_log,
                 "SBCBuffer scheduler start");
  Status s;
  ...
  while (running) {
    ...
    // 检查每个IO请求是否完成
    for (size_t i = 0; i < io_ctx_submitted_.size(); i++) {
      wait = false;
      ...
      if(ret > 0) {
        // 请求完成了
        // ROCKS_LOG_INFO(db_options_.info_log,
        //          "Finished AIO operation in SBCBuffer: %" PRIu64
        //          " , size: %" PRIu64 ", IO depth: %" PRIu32 "\n",
        //         io_ctx_submitted_[i]->file_number_, ret, GET_IO_DEPTH); // FIXME: 这东西会产出一个 , Unknown ???
        IO_CNT_SUB(1);
        ...
        wait = false;
        // std::cout << "Finished AIO operation in SBCBuffer: " << io_ctx_submitted_[i]->file_number_ << ", size: " << ret <<"\n";
      } else if(ret == -1) {
        printf("AIO operation failed.\n");
      }
    }
    ...
    if(GET_IO_DEPTH > MAX_SCHEDULE_IO_DEPTH) {
      ...
      for (size_t i = io_ctx_submitted_.size() - 1; io_ctx_submitted_.size() && GET_IO_DEPTH > MAX_SCHEDULE_IO_DEPTH && i > 0; i++) {
        if (aio_cancel(io_ctx_submitted_[i]->cb_->aio_fildes, io_ctx_submitted_[i]->cb_) == AIO_CANCELED) {
          // 取消成功，把请求放回worker队列
          ...
          IO_CNT_SUB(1);
        } else if (aio_error(io_ctx_submitted_[i]->cb_) == EINPROGRESS) {
            // 已经不能取消了
        } else {
            perror("aio_error");
    ...
    if (batch_size) {
      ...
      for (size_t i = 0; i < batch_size; i++) {
        ...
        cb->aio_offset = io_ctx_list_work_[i]->write_offset_;
      ...
      if (lio_listio(LIO_NOWAIT, aio_array, batch_size, nullptr) == -1) {
        perror("lio_listio");
        ROCKS_LOG_INFO(db_options_.info_log,
                  "SBCBuffer scheduler lio_listio err");
        for (size_t i = 0; i < batch_size; i++) {
  ...
  ROCKS_LOG_INFO(db_options_.info_log,
                 "SBCBuffer scheduler stopped, Wake up times: %" PRIu64
