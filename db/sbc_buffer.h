...
#define MAX_SCHEDULE_BATCH_SIZE 10ll
#define MAX_SCHEDULE_IO_DEPTH   64ll
...
struct SBCIOContex{
  ...
  uint64_t write_offset_;          // From WritableFileWriter

  IOStatus s_;                         // From SBCBuffer
  double priority_;                    // From SBCBuffer
  uint64_t submit_time_;               // From SBCBuffer
  struct aiocb *cb_ = nullptr;         // From SBCBuffer
  SBCIOState io_state_ = kInit;        // From SBCBuffer
  port::CondVar *buffer_cv_ = nullptr; // From SBCBuffer
...
public:
  // SBCBuffer() = default;
  explicit SBCBuffer(uint64_t buffer_size, const ImmutableDBOptions& db_options):
  ...
  SBCBuffer(SBCBuffer&&) = delete;
  SBCBuffer(const SBCBuffer&) = delete;
  SBCBuffer& operator=(const SBCBuffer&) = delete;
